...
export default function Card({project}) {
    ...
    return (
        <Container fluid className={darkMode ? "dark-mode container " : "white-mode container "}>
            <Row className={darkMode ? "mt-5 card-container-dark" : "mt-5 card-container-light"}>
                ...
                <motion.div animate={{ x: 0 }} initial={{x: 700}} transition={{duration:1}}>
                    <h4 className="card-title text-color-secondary">{name}</h4>
                    <p className="card-description">{description}</p>
                    <div>
                        <Button variant={darkMode ? "outline-info" :"info"} className="mt-3 mb-2 btn">
                            <a href={gitpages_url} target="_blank" rel="noreferrer">Live Demo</a>
                        </Button>
                        <Button variant={darkMode ? "outline-info" :"info"} className="mt-3 mb-2 btn">
